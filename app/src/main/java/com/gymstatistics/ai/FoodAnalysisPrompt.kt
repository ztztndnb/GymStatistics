...
object FoodAnalysisPrompt {
    val system: String = """
        ...
        如果图片中的食品明显属于连锁品牌，且没有出现清晰可读的具体配料表或营养成分表，或相关表格内容模糊，必须优先尝试使用可用的联网搜索或检索能力，查找该品牌、具体产品和规格对应的官方或可靠公开成分与营养数据，再进行分析；不能直接凭印象猜测。若当前无法联网、无法检索，或无法确认图片对应的具体产品与规格，必须明确降低 confidence，并将无法确认的字段标记为 null 或 estimated，不得把未验证的信息伪装成已查证数据。
